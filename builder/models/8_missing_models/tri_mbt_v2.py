...
from builder.models.src.transformer.mbt_encoder import TrimodalTransformerEncoder_Multitokens_MBT
...
class TRI_MBT_V2(nn.Module):
    def __init__(self, args):
        ...
        ##### Fusion Part
        self.fusion_transformer = TrimodalTransformerEncoder_Multitokens_MBT(
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, img, missing, f_indices):
        ...
        # final_output_mean = torch.mean(torch.stack([outputs[i][:, self.bottlenecks_n, :] for i in range(self.n_modality)]), dim=0)
        # final_output = torch.where(txt_lengths.unsqueeze(1) == 0, outputs[0][:, self.bottlenecks_n, :], final_output_mean)
        if self.multitoken == 0:
            ...
            final_output = torch.stack([tri_mean, vsltimg_mean, vslttxt_mean, outputs[0][:, 0, :]])
            final_output = final_output[missing, self.idx_order, :]
        else:
            outputs_stack = torch.stack([outputs[i][:, 0, :] for i in range(self.n_modality)])
            tri_mean = torch.mean(outputs_stack, dim=0)
