...
class TRI_MBT_VNOSHNOAVGTR(nn.Module):
    def __init__(self, args):
        ...
        nn.Linear(in_features=self.model_dim, out_features= self.output_dim,  bias=True)) for i in range(self.n_modality)])        
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, img, missing, f_indices, img_time, txt_time, flow_type, reports_tokens, reports_lengths):
        ...
        classInput = self.layer_norms_after_concat(outputs_stack)
        
        if self.args.vslt_type != "QIE":
            classInput = torch.cat([classInput, demo_embedding.unsqueeze(0).repeat(3,1,1)], dim=2)
        if "rmse" in self.args.auxiliary_loss_type:
