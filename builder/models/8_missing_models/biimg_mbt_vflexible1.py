...
class BIIMG_MBT_VFLEXIBLE1(nn.Module):
    def __init__(self, args):
        ...
        ##### Classifier
        self.flexibleavg = nn.Parameter(torch.zeros(2,1))
        self.flexsoft = nn.Softmax(dim=0)
        flexsoft_vi = torch.zeros(2).cuda() 
        flexsoft_vi[:] = 1
        flexsoft_vi = flexsoft_vi.le(0.5)
        flexsoft_v = torch.zeros(2).cuda() 
        flexsoft_v[:] = 1
        flexsoft_v[1] = 0
        flexsoft_v = flexsoft_v.le(0.5)
        self.flexsoft_masks = torch.stack([flexsoft_vi, flexsoft_v])
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, img, missing, f_indices, img_time, txt_time, flow_type, reports_tokens, reports_lengths):
        ...
        outputs, _ = self.fusion_transformer(enc_outputs = [vslt_embedding, img_embedding], 
                                        ...
                                        varying_lengths = [input_lengths, img_time],
                                        fusion_idx = None,
                                        missing=missing
                                        )
        ...
        outputs_stack = torch.stack([outputs[0][:, 0, :], outputs[1][:, 0, :]]) # vslt, img, txt
        outputs_stack = self.layer_norms_after_concat(outputs_stack)
        
        demo_embedding = demo_embedding.unsqueeze(0).repeat(2,1,1)
        ...
        all_cls_stack = torch.stack([vsltimg_mean, outputs_stack[0, :, :]])
