...
from builder.models.src.transformer.mbt_encoder import MultimodalTransformerEncoder_MBTSYM
from builder.models.src.transformer.module import LayerNorm

class MBT_MULTITOKEN_V4L(nn.Module):
    def __init__(self, args):
        ...
        ##### Fusion Part
        self.fusion_transformer = MultimodalTransformerEncoder_MBTSYM(
        ...
        self.predictor = nn.Sequential(
            nn.Linear(in_features=self.model_dim, out_features= self.model_dim, bias=False),
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, img, missing, f_indices):
        ...
        ### sim ###
        # p1 = self.predictor(context_vector[0]) # Full_modal token
        full_modal_token = context_vector[0].detach().clone()
        p2 = self.predictor(context_vector[1]) # missing modal token
        cos_loss = -self.cos_loss(full_modal_token, p2)
        ### sim ###
        ...
        return outputs, torch.mean(cos_loss)