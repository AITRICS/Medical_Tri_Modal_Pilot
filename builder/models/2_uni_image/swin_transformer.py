...
class SWIN_TRANSFORMER(nn.Module):
    def __init__(self, args):
        ...
        self.swin_transformer     = SWIN_TRANSFORMER_MODEL(img_size=224, patch_size=4, in_chans=1, num_classes=1,
                            embed_dim=32, depths=[2, 2, 2, 2], num_heads=[2, 4, 8, 16],
                            window_size=7, mlp_ratio=4., qkv_bias=True,
                            drop_rate=0.1, attn_drop_rate=0., drop_path_rate=0.1,
                            norm_layer=nn.LayerNorm, ape=False, patch_norm=True, enc_bool=False,
                            use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0])
        self.sigmoid = nn.Sigmoid()


    def forward(self, x):
        y_hat = self.swin_transformer(x)
