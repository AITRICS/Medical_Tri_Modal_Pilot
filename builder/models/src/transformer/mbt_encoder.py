...
class TrimodalTransformerEncoder_Multitokens_MBTVSLTMAIN(nn.Module):
    """
    ...
    def __init__(self,
        ...
        super(TrimodalTransformerEncoder_Multitokens_MBTVSLTMAIN, self).__init__()
        ...
        self.final_cls_tokens_num = 4
        self.cls_token = nn.Parameter(torch.randn(1, self.final_cls_tokens_num, d_model))
        ...
        self.i_bottleneck_mask = torch.zeros(12,12).cuda()              
        self.i_bottleneck_mask[::12, :12] = 1
        ...
        self.t_bottleneck_mask = torch.zeros(12,12).cuda()              
        self.t_bottleneck_mask[:12, :12] = 1
    ...
    def forward(self, enc_outputs, fixed_lengths = None, varying_lengths = None, return_attns = False, fusion_idx = None, missing = None):
        cls_token = self.cls_token.repeat(enc_outputs[0].size(0), 1, 1)
        # bottlenecks_list = [bottleneck_token.repeat(enc_outputs[0].size(0), 1, 1) for bottleneck_token in self.bottlenecks]
        bottlenecks_first = self.bottlenecks.repeat(enc_outputs[0].size(0), 1, 1)
        bottleneck_mask_list = [bottleneck_mask.repeat(enc_outputs[0].size(0), 1, 1) for bottleneck_mask in self.bottleneck_mask_list]
        enc_inputs = [torch.cat([cls_token, enc_outputs[idx]], axis=1) if idx == 0 else enc_outputs[idx] for idx, enc_input in enumerate(enc_outputs)]
        self_attn_masks = []
        bottleneck_self_attn_masks = []
        varying_lengths[0] += self.final_cls_tokens_num
        fixed_lengths[0] += self.final_cls_tokens_num
        varying_lengths[self.txt_idx][varying_lengths[self.txt_idx] == 3] = 0
                    ...
                    if len(bottleneck_self_attn_masks) < self.n_modality:
                        if self.mask[modal_idx]:
                            ...
                            if modal_idx == 0:
                                ...
                                b_mask[:,:12,:12] = bottleneck_mask_list[modal_idx][:,:,:]
...
class TrimodalTransformerEncoder_MBT_VSLTMAIN(nn.Module):
    """
    Based on "Attention Bottlenecks for Multimodal Fusion" from NeurIPS 2021
    by Arsha Nagrani, Shan Yang, Anurag Arnab, Aren Jansen, Cordelia Schmid, Chen Sun
    https://arxiv.org/abs/2107.00135
    ...
    def __init__(self,
        ...
        super(TrimodalTransformerEncoder_MBT_VSLTMAIN, self).__init__()
        ...
        # CLASSIFICATION TOKENS
        self.cls_token_per_modality = nn.ParameterList([nn.Parameter(torch.randn(1, 1, d_model)) for _ in range(self.cls_num)])
        self.bottlenecks = nn.Parameter(torch.randn(1, bottlenecks_n, d_model))
    ...
    def forward(self, enc_outputs, fixed_lengths = None, varying_lengths = None, return_attns = False, fusion_idx = None, missing = None):
        ...
        for n_modal in range(self.n_modality):
            if n_modal == 0:
                varying_lengths[n_modal] += self.cls_num
                fixed_lengths[n_modal] += self.cls_num
