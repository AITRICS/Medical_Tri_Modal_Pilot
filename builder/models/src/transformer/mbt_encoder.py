...
class TrimodalTransformerEncoder_Multitokens_MBT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.multitoken == 1:        
            ...
            self.final_cls_tokens_num = [1, 1, 1]
        ...
        if self.multitoken == 1:
            ...
            self.v_bottleneck_mask[4:12, :4] = 1
            self.v_bottleneck_mask[:4, 4:12] = 1
            self.v_bottleneck_mask[4:8, 8:12] = 1
            self.v_bottleneck_mask[8:12, 4:8] = 1
        ...
        if self.multitoken == 1:
            ...
            self.i_bottleneck_mask[4:12, :4] = 1
            self.i_bottleneck_mask[:4, 4:12] = 1
            self.i_bottleneck_mask[4:8, 8:12] = 1
            self.i_bottleneck_mask[8:12, 4:8] = 1
        ...
        if self.multitoken == 1:
            ...
            self.t_bottleneck_mask[4:12, :4] = 1
            self.t_bottleneck_mask[:4, 4:12] = 1
            self.t_bottleneck_mask[4:8, 8:12] = 1
            self.t_bottleneck_mask[8:12, 4:8] = 1
        ...
        for idx, enc_layers in enumerate(self.layer_stacks):
            ...
            if idx < self.fusion_idx:
                ...
                for modal_idx, enc_layer in enumerate(enc_layers):
                    ...
                    if len(bottleneck_self_attn_masks) < self.n_modality:
                        if self.mask[modal_idx]:
                            b_mask = get_attn_pad_mask(b_enc_output, varying_lengths[modal_idx]+bottlenecks.size(1), b_enc_output.size(1))
                ...
                for b_idx, b_output in enumerate(bottleneck_outputs):                
                    ...
                    bottlenecks_list.append(all_possible_b_means[missing, self.idx_order, :, :])
...
class MultimodalTransformerEncoder_MBT(nn.Module):
    """
    Based on "Attention Bottlenecks for Multimodal Fusion" from NeurIPS 2021
    by Arsha Nagrani, Shan Yang, Anurag Arnab, Aren Jansen, Cordelia Schmid, Chen Sun
    https://arxiv.org/abs/2107.00135
    ...
    def __init__(self,
            ...
            mask: list = [True, True]):
        super(MultimodalTransformerEncoder_MBT, self).__init__()
    ...
    def forward(self, enc_outputs, fixed_lengths = None, varying_lengths = None, return_attns = False, fusion_idx = None):
        ...
        for idx, enc_layers in enumerate(self.layer_stacks):
            ...
            if idx < self.fusion_idx:
                ...
                bottleneck_outputs_stack = torch.stack(bottleneck_outputs)
                bottlenecks_mean = torch.mean(bottleneck_outputs_stack, dim=0)
                bottlenecks = torch.where(varying_lengths[1].unsqueeze(1).unsqueeze(1) == 0, bottleneck_outputs[0], bottlenecks_mean)
                
        return enc_outputs, 0

class MultimodalTransformerEncoder_MBTSYM(nn.Module):
    """
    Based on "Attention Bottlenecks for Multimodal Fusion" from NeurIPS 2021
    by Arsha Nagrani, Shan Yang, Anurag Arnab, Aren Jansen, Cordelia Schmid, Chen Sun
    https://arxiv.org/abs/2107.00135
    ...
    def __init__(self,
            ...
            use_pe: list = [True, True],
            mask: list = [True, True]):
        super(MultimodalTransformerEncoder_MBTSYM, self).__init__()
    ...
    def forward(self, enc_outputs, fixed_lengths = None, varying_lengths = None, return_attns = False, fusion_idx = None):
        ...
        for n_modal in range(self.n_modality):
            ...
            if n_modal == self.txt_idx:
                    varying_lengths[n_modal][varying_lengths[n_modal] == 3] = 0
            if self.mask[n_modal]:
                mask = get_attn_pad_mask(enc_inputs[n_modal], varying_lengths[n_modal], fixed_lengths[n_modal])
