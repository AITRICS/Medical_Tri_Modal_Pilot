...
def _vision_transformer(
    ...
    **kwargs: Any,
    ...
    model.conv_proj = nn.Conv2d(1, 768, kernel_size=(16, 16), stride=(16, 16))# 64 = self.inplanes
    if weights is not None:
        mod_weights = weights.get_state_dict(progress=progress)
        mod_weights["conv_proj.weight"] = torch.nn.Parameter(torch.sum(mod_weights["conv_proj.weight"], dim=1, keepdim=True))
    
    #print(weights.get_state_dict(progress=progress))
    ...
    # model.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)# 64 = self.inplanes
    # mod_weights = weights.get_state_dict(progress=progress)
    # mod_weights["conv1.weight"] = torch.nn.Parameter(torch.sum(mod_weights["conv1.weight"], dim=1, keepdim=True))
...
def vit_b_16_m(*, weights: Optional[ViT_B_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_b_32_m(*, weights: Optional[ViT_B_32_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_l_16_m(*, weights: Optional[ViT_L_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_l_32_m(*, weights: Optional[ViT_L_32_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_h_14_m(*, weights: Optional[ViT_H_14_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
model_urls = _ModelURLs(
    {
        "vit_b_16_m": ViT_B_16_Weights.IMAGENET1K_V1.url,
        "vit_b_32_m": ViT_B_32_Weights.IMAGENET1K_V1.url,
        "vit_l_16_m": ViT_L_16_Weights.IMAGENET1K_V1.url,
        "vit_l_32_m": ViT_L_32_Weights.IMAGENET1K_V1.url,
    }
)