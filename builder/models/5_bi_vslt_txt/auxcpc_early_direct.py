...
class AUXCPC_EARLY_DIRECT(nn.Module):
    def __init__(self, args):

        super(AUXCPC_EARLY_DIRECT, self).__init__()
        ...
        self.ct_weight = nn.Parameter(torch.zeros((12,256,16)), requires_grad=True)
    ...
    def forward(self, x_raw, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, f_indices):
        ...
        ############################### Encoder ################################
        age = age.unsqueeze(1).unsqueeze(2).repeat(1, x_raw.size(1), 1)
        gen = gen.unsqueeze(1).unsqueeze(2).repeat(1, x_raw.size(1), 1)
        x = torch.cat([x_raw, age, gen], axis=2)
        ...
        if x.size(1) > t_samples:
            if self.neg_sample_from == "Future":        
                ...
                enc_samples = x_raw[:,t_samples:t_samples+12,:].contiguous()
        ...
        if x.size(1) > t_samples:
            ...
            if self.aux_loss == 1:              # cpcWbrl (0~10)
                total = torch.mm(pred.view(-1,16), torch.transpose(enc_samples.view(-1,16), 0, 1))
