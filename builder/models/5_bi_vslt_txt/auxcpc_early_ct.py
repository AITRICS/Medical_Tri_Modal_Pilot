...
class AUXCPC_EARLY_CT(nn.Module):
    def __init__(self, args):

        super(AUXCPC_EARLY_CT, self).__init__()
        ...
        self.c_t_Model = BimodalTransformerEncoder(
        ...
        self.layer_norms_after_concat_mirror = nn.LayerNorm(self.model_dim)
        ...
        self.ct_weight = nn.Parameter(torch.zeros((12,256,256)), requires_grad=True)
        torch.nn.init.uniform_(self.ct_weight, a=-math.sqrt(256), b=math.sqrt(256))
        ...
        if "cpc" in self.use_BRL:
            ...
            self.cosine_loss = torch.nn.CosineSimilarity(dim=0, eps=1e-07)
        ...
        self.output_lengths = torch.Tensor([13]).to(torch.int32).expand(self.batch_size)
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, f_indices):
        ...
        ################################# G_ar #################################
        final_input = torch.cat([z, txt_embedding], 1)
        c_t, _ = self.c_t_Model(final_input, 
            first_size = z.size(1),
        ...
        if x.size(1) > t_samples:
            ...
            final_input_mirror = torch.cat([enc_samples, txt_embedding], 1)
            c_t_mirror, _ = self.c_t_Model(final_input_mirror, 
            first_size = z.size(1),
            first_lengths = self.output_lengths,
            second_lengths = txt_lengths + 2
            )
            c_t_mirror = c_t_mirror[:, 0, :]
            c_t_mirror = self.layer_norms_after_concat_mirror(c_t_mirror)

            if self.aux_loss == 2:            # very small on both cosineWbrl and cosine (-1~1)
                loss = -torch.mean(self.cosine_loss(c_t.view(-1), c_t_mirror.view(-1)))
            elif self.aux_loss == 3:            # l2 very high (ex.86000) and good enough for l2Wbrl (ex.1.5)
                loss = torch.mean(self.l2_loss(c_t.view(-1), c_t_mirror.view(-1)))
