...
class AUXCPC_MBT_CT(nn.Module):
    def __init__(self, args):

        super(AUXCPC_MBT_CT, self).__init__()
        ...
        self.layer_norms_after_concat_mirror = nn.LayerNorm(self.model_dim * 2)
        ...
        self.output_lengths = torch.Tensor([12]).to(torch.int32).expand(self.batch_size)
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths, f_indices):
        ...
        if x.size(1) > t_samples:
            ...
            output_mirror, _ = self.c_t_Model([enc_samples, txt_embedding], 
                                      lengths = [self.output_lengths, txt_lengths + 2])
            c_t_mirror = torch.cat([output_mirror[i][:, 0, :] for i in range(self.n_modality)], dim=1)
