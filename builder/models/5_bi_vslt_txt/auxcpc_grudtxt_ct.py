...
class AUXCPC_GRUDTXT_CT(nn.Module):
    def __init__(self, args):

        super(AUXCPC_GRUDTXT_CT, self).__init__()
        self.args = args
        ...
        self.layer_norms_after_concat_mirror = nn.LayerNorm(self.model_dim * 2 + 2)
    ...
    def forward(self, x1, h, m1, d1, x_m, age, gen, input_lengths, txts, txt_lengths, f_indices):
        ...
        ############################ Auxiliary Loss ############################
        if x1.size(1) > t_samples:
            vslt_embedding_future = torch.stack(grud_output2, dim=1) # torch.Size([128, 36, 256])
            vslt_embedding_future = vslt_embedding_future[:, -1, :] # torch.Size([128, 256])
            c_t_mirror = torch.cat((vslt_embedding_future, txt_cls_output, ageTensor, genTensor), 1)
