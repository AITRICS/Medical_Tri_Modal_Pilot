...
class TXT_TRANSFORMER_TOCONCAT(nn.Module):
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths):
        ...
        vslt_embedding = vslt_embedding[torch.arange(x.shape[0]), txt_lengths]
        
        txts = txts.type(torch.IntTensor).to(self.device)
        txt_embedding = self.txt_embedding(txts)
        txt_output, _ = self.transformer_encoder(txt_embedding, input_lengths = txt_lengths+1)
        txtClsOutput = txt_output[:, 0]
        
        ageTensor = age.unsqueeze(1)
        genTensor = gen.unsqueeze(1)

        classInput = torch.cat((vslt_embedding, txtClsOutput, ageTensor, genTensor), 1)
