...
class CROSS_TRANSFORMER_TXT_VSLT_SCSCSC(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args

        self.model_dim = args.transformer_dim
        self.txt_num_layers = 4
        self.txt_num_heads = 4
        self.txt_dropout = 0.1

        self.self_cross_num_layers = args.cross_transformer_num_layers
        self.self_cross_num_heads = args.cross_transformer_num_head
        self.self_cross_model_dim = args.cross_transformer_dim
        self.self_cross_dropout = 0.1
        ...
        self.init_fc = nn.Linear(self.num_nodes + 2, self.self_cross_model_dim)
        ...
        self.self_cross_attn_encoder = SelfCrossTransformerEncoder(
            d_input = self.self_cross_model_dim,
            n_layers = self.self_cross_num_heads,
            n_head = self.self_cross_num_heads,
            d_model = self.self_cross_model_dim,
            d_ff = self.self_cross_model_dim * 2,
            dropout = self.self_cross_dropout,
            pe_maxlen = 500,
            use_pe = True,
            classification = False
        )
        ...
        for _ in range(12):
            ...
            nn.Linear(in_features=self.self_cross_model_dim, out_features= 64, bias=True),
            nn.BatchNorm1d(64),
        ...
        self.cls_tokens = nn.Parameter(torch.zeros(1, 1, self.self_cross_model_dim))
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths):
        ...
        # print("vslt len: ", input_lengths)
        # print("#####cross#####")
        # print("txt_output: ", txt_output[0,:,0])
        enc_output, _ = self.self_cross_attn_encoder(padded_q_input = emb, 
                                            padded_kv_input = txt_output, 
                                            q_input_lengths = input_lengths,
                                            q_input_length = emb.size(1),
                                            kv_input_lengths = txt_lengths)
        
        
        finalClsOutput = enc_output[:, 0]
