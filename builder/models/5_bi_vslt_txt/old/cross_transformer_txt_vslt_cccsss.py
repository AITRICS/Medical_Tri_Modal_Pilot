...
class CROSS_TRANSFORMER_TXT_VSLT_CCCSSS(nn.Module):
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths):
        ...
        # print("#####cross#####")
        cross_enc_output, _ = self.cross_attn_encoder(padded_q_input = emb, 
                                            padded_kv_input = txt_output, 
                                            q_input_lengths = emb.size(1),
                                            kv_input_lengths = txt_lengths)
        
        # print("#####self#####")
        output, _ = self.self_attn_encoder(cross_enc_output,
                                           input_lengths = input_lengths)
        
        finalClsOutput = output[:, 0]
