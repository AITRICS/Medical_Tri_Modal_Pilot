...
class TXT_TRANSFORMER_VSLT_FEATURE_TOTRANSFORMER(nn.Module):
    ...
    def forward(self, x, h, m, d, x_m, age, gen, input_lengths, txts, txt_lengths):
        ...
        finalInputs = torch.cat((vslt_embedding, txt_output), dim=1)
        finalOutput, _ = self.final_encoder(finalInputs, input_lengths = txt_lengths + vslt_embedding.shape[1] + 1)
