...
class MULTI_GRU_D_T_TRANSFORMER(nn.Module):
        def __init__(self, args):
                ...
                self.hidden_decay = nn.Linear(input_size, enc_model_dim-2)

                self.gru = nn.GRUCell(input_size*2, enc_model_dim-2, True)
        ...
        def forward(self, x, h, m, d, x_m, age, gen, input_lengths):
                ...
                age = age.unsqueeze(1).unsqueeze(2).repeat(1, ori_length, 1)
                gen = gen.unsqueeze(1).unsqueeze(2).repeat(1, ori_length, 1)
                emb = torch.cat([emb, age, gen], axis=2)
