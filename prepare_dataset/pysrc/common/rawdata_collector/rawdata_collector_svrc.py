...
class RawdataCollector:
    ...
    def read_patient_file(self):
        in_file = os.path.join(self.arg.in_dir, self.chid, 'PERSON_SurrogateKey.pkl')
    ...
    def read_transfer_file(self) -> List[Transfer]:
        ...
        in_file = os.path.join(self.arg.in_dir, self.chid, 'TIME_SurrogateKey.pkl')
        if not os.path.exists(in_file):
            return transfers
    ...
    def read_operation_file(self) -> List[Operation]:
        ...
        in_file = os.path.join(self.arg.in_dir, self.chid, 'OPList_SurrogateKey.pkl')
        if not os.path.exists(in_file):
            return operations
    ...
    def read_observation_file(self, feature_times):
        ...
        for in_file_base in in_file_bases:
            ...
            for feature_item in d:
                ...
                feature_time = str2datetime(feature_item['ActDt'])
    ...
    def read_labresult_file(self, feature_times):
        in_file = os.path.join(self.arg.in_dir, self.chid, 'Lab2016_SurrogateKey.pkl')
        if not os.path.exists(in_file):
            return feature_times
    ...
    def read_labresult_new_file(self, feature_times):
        in_file = os.path.join(self.arg.in_dir, self.chid, 'SEPSIS_LAB_New_SurrogateKey_utf8.pkl')
        if not os.path.exists(in_file):
            return feature_times
        ...
        for feature_item in d:
            ...
            if context_id not in FEATURE_FOR_DEFINE_PATIENT.LAB_NEW:
                continue

            feature_code = FEATURE_FOR_DEFINE_PATIENT.LAB_NEW[context_id]
