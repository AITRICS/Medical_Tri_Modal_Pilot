...
# @Last Modified by:   Destin
# @Last Modified time: 2022-05-18 
...
def main():
    ...
    if ARG.dataset == 'MIMIC':
        ...
        if not os.path.exists('MIMICED_LOG'):
            os.mkdir('MIMICED_LOG')

        for (file_name, missing_ids) in missing_subject_id:
            writer = open('MIMICED_LOG/{}_missing_id.txt'.format(file_name.replace('.csv', '')),'w')
